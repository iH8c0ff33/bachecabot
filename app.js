...
var db = new Database('postgres://bot@telegram_database:5432/telegram');
        ...
        if (req.data.action.login) {
          if (req.body.message.entities) {
            if (req.body.message.text.slice(req.body.message.entities[0].offset,
              ...
              return req.data.update({action: req.data.action}).then(function () {
                bot.sendMessage({
                  chat_id: req.user.id,
          ...
          if (req.data.action.login.insert == 'school') {
            req.data.action.login.school = req.body.message.text;
            req.data.action.login.insert = 'username';
            return req.data.update({action: req.data.action}).then(function (updatedData) {
              bot.sendMessage({
                chat_id: req.user.id,
            ...
            req.data.action.login.username = req.body.message.text;
            req.data.action.login.insert = 'password';
            return req.data.update({action: req.data.action}).then(function (updatedData) {
              bot.sendMessage({
                chat_id: req.user.id,
                text: 'Insert field '+updatedData.action.login.insert+' or type /cancel to cancel login'
            ...
            req.data.action.login.password = req.body.message.text;
            req.data.action.login.insert = 'confirm';
            return req.data.update({action: req.data.action}).then(function (updatedData) {
            ...
            if (req.body.message.text.search(/yes/i) > -1) {
              return bot.sendMessage({
                ...
                req.chat.createCredential({
                  ...
                  req.data.action.login = undefined;
                  req.data.update({action: req.data.action}).then(function (updatedData) {
                    bot.sendMessage({
